...
<div class="main-wrapper px-8">
    ...
    <div class="divide-y divide-zinc-400 content-wrapper flex flex-col gap-y-4">
        <?php
        include "../../backend/php/koneksi.php";
        $qry_produk = mysqli_query($conn, "SELECT * FROM `lelang` inner JOIN barang USING (id_barang) WHERE id_masyarakat = $_SESSION[id_masyarakat] and status like 'buka' ");
        ...
        ?>
            <tr>
                <?php if (isset($lelang)) : ?>
                    <?php
                    include "../../backend/php/koneksi.php";
                    $qry_select_max_price = mysqli_query($conn, "SELECT *  FROM `lelang` WHERE id_barang = $lelang[id_barang] and harga_akhir in (SELECT MAX(harga_akhir) from `lelang` WHERE id_barang = $lelang[id_barang])");
                    $get_harga_tertinggi = mysqli_fetch_array($qry_select_max_price);
                    ?>
                    <div class="content-wrapper w-full flex flex-row justify-between items-center">
                        <div class="wrapper-detail w-full">
                            <div class="left-content flex flex-row items-center gap-x-8 w-full">
                                <div class="img">
                                    <img src="../<?= $lelang['foto_produk'] ?>" alt="" class="w-64 h-32 object-contain">
                                </div>
                                <div class="text font-bold flex w-full items-center">
                                    <div class="title text-sm text-zinc-600">
                                        <p>Highest bid user id: <?= $get_harga_tertinggi['id_masyarakat'] ?></p>
                        ...
                        <div class="wrapper-action w-full flex flex-row justify-between">
                            <div class="price">
                                ...
                                <p class="font-semibold text-sm">item price: Rp. <?= number_format($get_harga_tertinggi['harga_akhir'], 0, ",", '.') ?></p>
                            </div>
                            <div class="right-content  flex flex-row justify-evenly w-auto gap-x-8 font-semibold text-md items-center">
                                ...
                                <form action="bid_process.php" method="POST">
                                    <button name="get_id" value="<?= $lelang["id_barang"] ?>" class="bg-yellow-300 px-2 rounded-md py-1 tracking-widest">Bid Rp.<?= $lelang['kelipatan_bid'] ?></button>
                                </form>
                            </div>
                        </div>
                    </div>
